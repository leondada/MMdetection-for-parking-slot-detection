...
class ResizeforPsd:
    """
     目前只支持单尺度，未来需要增加多尺度.
     对于训练集，只resize图片，因为训练集图像预处理后点坐标是0到1的，无需改变
     对于测试集，既要resize图片，也要resize点坐标，因为测试集原始标签是绝对坐标。但是实现的时候读坐标是放在ap计算里，所以这里测试集的坐标可以不resize
    """
    ...
    #     # print('888888888888',results)
    #     results['gt_bboxes'] = np.array(results['gt_bboxes'])
    #     results['gt_bboxes'][:,4] *= results['scale_factor']
    #     img_shape = results['img_shape']
    #     results['gt_bboxes'][:,4][:, 0::2] = np.clip(results['gt_bboxes'][:,4][:, 0::2], 0, img_shape[1])
    #     results['gt_bboxes'][:,4][:, 1::2] = np.clip(results['gt_bboxes'][:,4][:, 1::2], 0, img_shape[0])
